...
class RecourseBuilder(object):
    ...
    def __init__(self, action_set, x = None, pwfeature_costs=None,**kwargs):
        """
        ...
        """
        self.pwfeature_costs = pwfeature_costs
    ...
    def _get_mip_build_info(self, cost_function_type = 'percentile', validate = True):
        ...
        for i, (n, a) in enumerate(actions.items()):
            print("actions", a)
            if len(a) >= 2:
                ...
                if self.mip_cost_type == "l2":
                    c = a**2
                    dc = c

                if self.mip_cost_type == "l1":
                    c = np.abs(a)
                    dc = c

                if self.mip_cost_type == "pfc":
                    c = self.pwfeature_costs[i]*np.abs(a)
                    dc = c
...
class _RecourseBuilderCPX(RecourseBuilder):
    ...
    def build_mip(self):
        ...
        # print(indices)
        if cost_type in ('total', 'local', "l2","l1", "pfc"):
